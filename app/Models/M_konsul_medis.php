...
class M_konsul_medis {
    ...
    function insert($params) {
        $sql = "INSERT
                INTO TAC_RI_KONSUL_MEDIS(FS_KD_REG, FS_KD_PEG, FS_KONSUL1, FS_KONSUL2, mdb, mdd_date, mdd_time)
                VALUES     (?,?,?,?,?,?,?)";
        return $this->db->query($sql, $params);
    }

    function update($params) {
        $sql = "UPDATE TAC_RI_KONSUL_MEDIS SET FS_KD_PEG =?,FS_KONSUL1=?, FS_KONSUL2=? WHERE FS_KD_TRS = ?";
        return $this->db->query($sql, $params);
    }

    function delete($params) {
        $sql = "DELETE FROM TAC_RI_KONSUL_MEDIS WHERE FS_KD_TRS = ?";
        return $this->db->query($sql, $params);
    }
    ...
    function get_data_konsul_by_rg($params) {
        $sql = "SELECT a.*,b.FS_NM_PEG 
                FROM TAC_RI_KONSUL_MEDIS a
    ...
    function get_data_konsul_by_trs($params) {
        $sql = "SELECT a.*,b.FS_NM_PEG 
                FROM TAC_RI_KONSUL_MEDIS a
                LEFT JOIN HOSPITAL.dbo.TD_PEG b ON a.FS_KD_PEG=b.FS_KD_PEG
                WHERE FS_KD_TRS = ?
    ...
    function get_pasien_bangsal_by_dokter($params) {
        $sql = " SELECT     aa.fs_kd_reg, RIGHT(bb.fs_mr,8) 'FS_MR', 
            ISNULL(ee.fs_nm_bed, ' ') fs_nm_bed, 
            ISNULL(cc.fs_nm_pasien, ' ') fs_nm_pasien,cc.fd_tgl_lahir,
            cc.fs_alm_pasien,hh.fs_nm_peg
